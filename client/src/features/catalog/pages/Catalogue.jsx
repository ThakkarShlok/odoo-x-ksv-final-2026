...
export default function Catalogue() {
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-6">
      ...
      <section className="overflow-hidden rounded-[1.75rem] border border-border bg-card shadow-sm">
        <div className="grid gap-6 px-5 py-6 sm:px-7 lg:px-8 lg:py-8 xl:grid-cols-[minmax(0,1.15fr)_minmax(24rem,0.85fr)] xl:items-end">
          <div className="space-y-3 xl:pr-4">
            <p className="text-[0.72rem] font-semibold uppercase tracking-[0.2em] text-primary">
              Zenith Rentals Catalogue
            </p>
            <div className="space-y-2">
              <h1 className="max-w-3xl text-3xl font-semibold tracking-tight text-foreground sm:text-[2.6rem] sm:leading-[1.05]">
                Real equipment, real photography, and availability you can
                trust.
              </h1>
              <p className="max-w-2xl text-sm leading-6 text-muted-foreground sm:text-base">
                Browse by category, narrow by rental window, and compare pricing
                without leaving the catalogue.
                {meta
                  ? ` ${meta.totalCount} product${meta.totalCount === 1 ? "" : "s"} are live right now.`
                  : ""}
              </p>
            </div>
          </div>

          <div className="grid gap-3 md:grid-cols-3 xl:min-w-0">
            <div className="rounded-2xl border border-border bg-background/70 px-4 py-3">
              <p className="text-[0.68rem] uppercase tracking-[0.18em] text-muted-foreground">
                Daily pricing
              </p>
              <p className="mt-2 text-lg font-semibold text-foreground">
                Front and center on every card
              </p>
            </div>
            <div className="rounded-2xl border border-border bg-background/70 px-4 py-3">
              <p className="text-[0.68rem] uppercase tracking-[0.18em] text-muted-foreground">
              ...
              <p className="mt-2 text-lg font-semibold text-foreground">
                {depositLabel ?? "Shown from active rules"}
              </p>
            </div>
            <div className="rounded-2xl border border-border bg-background/70 px-4 py-3">
              <p className="text-[0.68rem] uppercase tracking-[0.18em] text-muted-foreground">
              ...
              <p className="mt-2 text-lg font-semibold text-foreground">
                {windowed ? "Window-filtered stock" : "Current stock view"}
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-2 text-sm text-muted-foreground">
          ...
          {windowed
            ? "Availability badges reflect your selected rental dates."
            : "Add dates to switch from current stock to date-specific availability."}
        </div>
        ...
        <aside className="h-fit rounded-[1.5rem] border border-border bg-card p-5 shadow-sm xl:sticky xl:top-6">
          <div className="space-y-6">
              ...
              <div className="space-y-1.5">
                ...
                {categories.map((c) => (
                  ...
                  >
                    <span>{c.name}</span>
                    <span
                      className={`text-xs ${query.categoryId === c.id ? "text-primary-foreground/80" : "text-muted-foreground"}`}
                    >
                      {c.productCount}
                    </span>
                  </button>
            ...
            <div className="rounded-2xl border border-border bg-background p-3 text-sm text-muted-foreground">
              Availability badges switch from current stock to date-specific
              unit counts when both dates are applied.
            </div>
              ...
              <div className="grid gap-5 sm:grid-cols-2 2xl:grid-cols-3">
                {state.items.map((p) => {
                  ...
                  return (
                    <Card
                      key={p.id}
                      className="group flex h-full flex-col overflow-hidden rounded-[1.5rem] border-border bg-card shadow-sm transition-all duration-200 hover:-translate-y-0.5 hover:shadow-md"
                    >
                      <Link
                        to={`/app/product/${p.id}`}
                        state={{ product: p }}
                      ...
                      >
                        <div className="aspect-[5/4] overflow-hidden bg-white">
                          {p.primaryImage ? (
                            <img
                              src={assetUrl(p.primaryImage.url)}
                              alt={p.name}
                              loading="lazy"
                      ...
                      <CardContent className="flex flex-1 flex-col p-5">
                        <div className="flex items-start justify-between gap-3">
                          <div className="min-w-0">
                            ...
                            <h3 className="mt-1 line-clamp-2 min-h-[3.25rem] text-lg font-semibold leading-6 text-foreground">
                              {p.name}
                            </h3>
                            <p className="mt-1 text-sm text-muted-foreground">
                              {p.brand ?? "Rental-ready unit"}
                            </p>
                          </div>
                          <ArrowUpRight
                        ...
                        <div className="mt-4 space-y-2 border-t border-border pt-4">
                          <div className="flex items-end justify-between gap-3">
                            <div>
                              ...
                              <p className="mt-1 text-2xl font-semibold tabular-nums text-foreground">
                                {p.rates?.DAILY ? money(p.rates.DAILY) : "—"}
                              </p>
                            </div>
                          ...
                          <div className="flex items-center justify-between text-sm text-muted-foreground">
                            ...
                            <span className="text-right">
                              {depositLabel ?? "Shown at checkout"}
                            </span>
                          </div>
                        </div>
              ...
              {meta && meta.totalPages > 1 ? (
                <div className="mt-6 flex items-center justify-center gap-3">
                  <Button
                    ...
                    onClick={() => patch({ page: query.page - 1 })}
                  >
                    Previous
                  </Button>
