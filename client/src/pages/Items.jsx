/**
 * Items page — the end-to-end round-trip made visible: axios → JWT → Express → Prisma → Postgres
 * → back to React. Lists items and creates them. On create, a persistent notification row appears
 * in the bell (server side) AND a transient toast fires here — the two feedback channels side by
 * side, on purpose.
 * Handles the three distinct async states explicitly (loading / error+retry / empty) via the
 * shared primitives, rather than conflating them.
 */
...
export default function Items() {
  const [state, setState] = useState({ status: 'loading', items: [], count: 0, error: null });
  ...
  async function onCreate(values) {
    try {
      ...
      toast.success(`Created "${created.name}".`); // transient feedback
      reset();
      load(); // refetch so the list and count reflect the new row (and the bell will poll it up)
    } catch (error) {
